...
export const { AddressZero: ZERO_ADDRESS, MaxUint256: MAX_UINT256 } = ethers.constants;
...
//---------------------Loading Base Contract-----------------------------

const getERC20 = async () => await ethers.getContractFactory("ERC20");
const getERC721 = async () => await ethers.getContractFactory("ERC721");

//---------------------Balance Helper Function---------------------------

export async function getBalance(address: string, tokenAddress: string = ZERO_ADDRESS): Promise<BN> {
  if (tokenAddress === ZERO_ADDRESS) return provider.getBalance(address);
  else return (await getERC20()).attach(tokenAddress).balanceOf(address);
}
...
export async function getTxInfo(transaction: Promise<TransactionResponse>): Promise<TransactionReceiptWithFee> {
  try {
    ...
    const transactionReceipt = await ethers.provider.getTransactionReceipt((error as TransactionReceipt).transactionHash);
...
export async function updateTxInfo(transaction: Promise<TransactionResponse>, onUpdate: (txInfo: TransactionReceiptWithFee) => any): Promise<TransactionResponse> {
...
export class BalanceTracker {
  ...
  static expect(transaction: Promise<TransactionResponse> | (() => Promise<TransactionResponse>)): Chai.Assertion {
    if (typeof transaction == "function") return expect(() => BalanceTracker.updateFee(transaction()));
  ...
  diff(snapshotNameA: string, snapshotNameB: string): Record<string, BalanceSnapshotDiff> {
    if (!(this.snapshots[snapshotNameA] && this.snapshots[snapshotNameB])) throw new Error("Snapshot is not found");

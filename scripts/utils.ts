...
export class Table extends CliTable3 {
  ...
  toObject(types: Table.ContractType[] | "all"): Record<string, string> {
    return this.reduce((result: any, row: any) => {
      if (types.includes(row[1]) || types == "all") result[`${row[0]}_${row[1]}`] = row[2];
      return result;
    }, {});
  }

  toArray(types: Table.ContractType[] | "all"): [string, Table.ContractType, string][] {
    return super.filter((row: any) => types.includes(row[1]) || types == "all") as [string, Table.ContractType, string][];
  }
  ...
  async save(destination: string, fileName: string): Promise<Record<string, string>> {
